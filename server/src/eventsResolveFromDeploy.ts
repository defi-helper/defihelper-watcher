              ...
              contracts.map(async ({ id, abi, network, address }) => {
                ...
                return Promise.all(
                  listeners.map(async (listener) => {
                    const cacheKey = `eventListener:${listener.id}:deployBlockNumber`;
                    const syncHeight = await cache.get(cacheKey).then((v) => Number(v ?? '0'));
                    if (syncHeight === 0) {
                      const oneYearInSeconds = 60 * 60 * 24 * 365;
                      const networkDetails = container.blockchain.byNetwork(contract.network);

                      const startBlock = Math.max(
                        0,
                        Math.round(
                          currentBlock - oneYearInSeconds / networkDetails.averageBlockTime,
                        ),
                      );

                      return cache.setex(cacheKey, 3600, startBlock);
                    }
